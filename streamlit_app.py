...
# streamlit_app.py (extra-robust v3)
import os
import numpy as np
import pandas as pd
...
# NEW: schema introspection for ranker pipeline
def get_training_schema(rank_pipe):
    """Extract which columns the fitted ColumnTransformer expects."""
    try:
...
def prepare_for_ranker(df, rank_pipe):
    """Ensure incoming scoring frame has the same columns/types the ranker saw at train time."""
    out = df.copy()
...
def load_csvs(folder):
    ...
    # required cols check (but we'll synthesize constraints & engagement if needed)
    for name, df, cols in [
    ...
    if "client_id" not in cns.columns:
        st.warning("constraints.csv has no 'client_id'. Creating synthetic constraints from client_profiles.")
    ...
    if "client_id" not in eng.columns:
        st.warning("engagement_logs.csv has no 'client_id'. Building empty notes for all clients.")
...
def build_retrieval_fallback(client_profiles, deal_metadata):
    # Very simple match score: sector + region pref matches (0..2 normalized to 0..1)
...
def build_retrieval(client_profiles, engagement_logs, deal_metadata, max_features=5000, ngram=(1,2)):
    ...
    # If all docs are empty (no tokens), fallback to simple matching
    ...
    if tokens_total == 0:
        st.info("Retrieval: empty text detected; using simple sector/region fallback.")
...
def build_training_table():
    cns = constraints
    if "client_id" not in cns.columns:
        st.warning("Synthesizing constraints in training table (no client_id).")
...
def build_ranker(X, y):
    ...
    if txt_col:
        try:
            ...
            txt_ok = False
    if not txt_ok:
        st.info("Ranking: no non-empty notes detected; skipping text features.")
        txt_col = None
