...
# Safe import: joblib is optional; app will still run (it can always retrain)
try:
    import joblib  # used only if you want to load a pre-trained model
...
# Build ranker always if no joblib available; otherwise load if exists and user disabled retrain
if retrain_in_app or (joblib is None):
    ranker, auc, ap = build_ranker(X, y)
else:
    try:
        ranker = joblib.load("lr_ranker.joblib")  # optional artifact
        auc = ap = None
    except Exception:
        ranker, auc, ap = build_ranker(X, y)
...
if auc is not None and ap is not None:
    ...
    st.sidebar.write("Loaded from artifact or joblib unavailable; trained in-app.")
